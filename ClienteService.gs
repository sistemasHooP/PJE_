...
var ClienteService = {
  ...
  buscarPorCPF: function(cpf) {
    ...
    var clientes = Database.findByCPF(CONFIG.SHEET_NAMES.CLIENTES, 'cpf', cpfNormalizado);
  ...
  cadastrar: function(payload) {
    ...
    var existente = this.buscarPorCPF(cpf);
    if (existente) {
    ...
    Utils.logAction(auth.user.email, ENUMS.ACOES_LOG.CRIAR_CLIENTE,
      'Cliente criado: ' + Utils.maskCPF(cpf));
